...
}:
{
  ...
  config =
    let
      ...
      bedTimeJobForTime =
        ...
        ++ (map (
          delayPassed:
          let
            warnTimeMinStr = builtins.toString (bedTimeMin + delayPassed);
          in
          (cronJobToSystemdTimer {
    ...
    in
    lib.mkIf config.dobikoConf.bedtime.enabled {
      environment.systemPackages = with pkgs; [ kdePackages.kdialog ];

      systemd.timers = lib.foldl (acc: x: acc // x) { } (builtins.map (x: x.timer) bedTimeJobs);
      systemd.services = lib.foldl (acc: x: acc // x) { } (builtins.map (x: x.service) bedTimeJobs);
    };
}
