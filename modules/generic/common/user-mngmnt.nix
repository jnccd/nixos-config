...
let
  ...
  mkGroup = index: user: {
    name = user.name;
    value = {
      gid = lib.mkIf (user ? defaultGid) user.defaultGid;
    };
  };
in
{
  ...
  config =
    let
      usersToDefine = config.dobikoConf.userMngmnt.additionalUsers ++ globalArgs.baseUsers;
    in
    {
    ...
    //
      ...
      (
        let
          dbAccessUsernames = (
            map (user: user.name) (builtins.filter (user: user ? dbAccess && user.dbAccess) usersToDefine)
          );
        ...
        in
        {
          sops.secrets = lib.mkIf config.dobikoConf.postgres.enabled (builtins.listToAttrs dbSopsSecrets);
                    ...
                    let
                      ...
                      createRoleScript = pkgs.writeScript "create-role-script" ''
                        ${pkgs.postgresql}/bin/psql -U postgres -c "DO \$\$ BEGIN
                          ...
                          END IF;
                        END \$\$;"'';
                      createDbScript = pkgs.writeScript "create-db-script" ''${pkgs.postgresql}/bin/psql -U postgres -c "CREATE DATABASE ${psqlUsername} OWNER ${psqlUsername};"'';
                    in
                    pkgs.writeScript "script" ''
                      sleep 1
                      export PSQL_PASS=$(cat ${config.sops.secrets."postgres/pass/${psqlUsername}".path})
